...
var config = {
	...
	"settings" : {
		 "SCRIPT_INSTANCE" : 1,
			...
			"SHEET_NAME" : "settings",
			"PROJECT_ID" : "biddy-io",
			"DATASET_ID" : "keyword_settings_peak_ace",
			"TRUNCATE_EXISTING_DATASET" : false,
			"TRUNCATE_EXISTING_TABLES" : false,
			"PARTITION_EXPIRATION" : 30,
			"KEY" : [ "AdGroupName", "Criteria" ],
			"IGNORED_LABELS" : [ "PA_duplicate_keyword" ],
			"MAILGUN" : {
...
var CONSTANTS = {
	SCRIPT_NAME : 'Keyword_Settings_Checker',
	EMPTY : '',
	ID_COLUMN : 1, // 0-based
	EMAIL_COLUMN : 4, // 0-based
	ACCOUNTS_LIMIT : 50,
...
function processAccount( params ){
	try{
		...
		if( computeCurrentCampaignIds( params.currentPart ).length == 0 ){
			_.log( account.getName() + ': The work is partitioned in NUM_PARTS parts.'
		...
		if( CHECK_NEW_ITEMS ){
			var count = 0;
			for ( var key in oldData ){
				// to find items in newData, which are not in oldData,
				// we need to delete oldData from newData
				delete newData[ key ];
				count++;
			}
			if( count > 0 ){
				// if there is no old data (count==0) then it is one of the initial runs 
				// - dont check for new items in this case
				for ( var key in newData ){
					compareRows(
						CONFIG.EXPECTED_CHECKS_FOR_NEW_ITEMS,
						newData[ key ],
						res.checkNew
					);
				}
...
function handleConflicts( check, campaignName, key, oldValue, newValue, target ){
	...
	//	Logger.log( check + ' ' + campaignName + ' ' + key + ' ' + oldValue + ' ' + newValue );
}

function formatResult( target, accountName, oldOrExpected, newOrFound ){
	var html = '<style>th, td { border: 1px solid #ddd;}</style>';
	
	html += '<table style="padding:10px;border-collapse:collapse;">';
	for( check in target ){
		
		html += '<tr> <td colspan="' + ( KEY.length + 1 ) + '"> <h2>' + accountName
			+ '</h2> </td> <td colspan="2"> <h3> ' + check + ' </h3> </td> </tr>';
		html += '<tr style="background-color:rgb(200,200,200);"> <td> Campaign </td>'
				+ '<td>' + KEY.join( '</td><td>' ) + '</td>'
				+ '<td> ' + oldOrExpected + ' value </td> <td> ' + newOrFound + ' value </td> </tr>';
		
		for( campaignName in target[ check ].campaign ){
			var campaignObj = target[ check ].campaign[ campaignName ];
			var firstRow = true;
			var countItems = Object.keys( campaignObj.item ).length;
			if( countItems != campaignObj.countItems ){
				Logger.log( 'WARNING: countItems don\'t match!' );
			}
			for( key1 in campaignObj.item ){
				var oldValue = campaignObj.item[ key1 ].oldValue;
				var newValue = campaignObj.item[ key1 ].newValue;
				
				html += '<tr>';
				if( firstRow ){
					html += '<td  rowspan="' + ( countItems + ( campaignObj.countOtherItems > 0 ? 1 : 0 ) )
						+ '"> ' + campaignName + ' </td>';
					firstRow = false;
				}
				html += '<td>' + key1.split( SPLITTER ).join( '</td><td>' ) + '</td>';
				html += '<td> ' + oldValue + ' </td> <td> ' + newValue + ' </td></tr>';
			}
			if( campaignObj.countOtherItems > 0 ){
				html += '<tr> <td> ... and ' + campaignObj.countOtherItems + ' other </td>'
					//+ KEY.join( '/' )
					+ ' <td>' + KEY.map( function( x ){ return CONSTANTS.EMPTY } ).join( '</td><td>' ) + '</td>'
					//+ ' <td>' + CONSTANTS.EMPTY + '</td><td>' + CONSTANTS.EMPTY + '</td>'
					+ '<td>' + CONSTANTS.EMPTY + '</td> </tr>';
			}
		}
		if( target[ check ].countOtherCampaigns > 0 ){
			html += '<tr> <td> ... and ' + target[ check ].countOtherCampaigns
				+ ' other campaigns </td> <td> ... and ' + target[ check ].countOtherItems + ' other ' + KEY.join( '/' )
				+ ' </td> <td>' + CONSTANTS.EMPTY + '</td><td>' + CONSTANTS.EMPTY + '</td> </tr>';
		}
	}
	html += '</table>';
...
function finalProcessing( results ){
	try{
		...
		for( email in emailMap ){
			...
			MAIL_APP.sendEmail( email, CONSTANTS.SCRIPT_NAME + '' + SCRIPT_INSTANCE + ' ' + AdWordsApp.currentAccount().getName(), null, html );
		}
	...
	for( var index in CONFIG.CHECKS ){
		...
		if( oldOrExpectedValue != newValue ){
			handleConflicts(
				check,
				newRow[ 'CampaignName' ],
				KEY.map( _.property( newRow )).join( SPLITTER ),
