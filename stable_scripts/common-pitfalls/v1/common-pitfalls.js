...
var VIEWS = {
	...
	ADGROUP_NEGATIVE_KEYWORD_CONFLICTS : [
		...
		'  GROUP BY customer.ExternalCustomerId, AccountName, campaign.CampaignName, CampaignId, AdGroupId, adgroup.AdGroupName, keyword.Criteria, keyword.KeywordMatchType, neg.Criteria, neg.KeywordMatchType',
		')',
		'WHERE TRUE',
		'  AND (',
	...
	ADGROUP_WITHOUT_AUDIENCE : [
		...
		'GROUP BY customer.ExternalCustomerId, CustomerDescriptiveName, campaign.CampaignName, adgroup.AdGroupName, CampaignId, AdGroupId',
		'HAVING TRUE',
	...
	CAMPAIGN_NEGATIVE_KEYWORD_CONFLICT : [
		...
		'    AND keyword._LATEST_DATE = keyword._DATA_DATE',
		'    AND neg._LATEST_DATE = neg._DATA_DATE',
		'  GROUP BY customer.ExternalCustomerId, AccountName, campaign.CampaignName, CampaignId, keyword.Criteria, keyword.KeywordMatchType, neg.Text, neg.MatchType',
		')',
		'WHERE TRUE',
		'  AND (',
		'       ( neg_match_type = \'EXACT\'  AND negative_keyword = keyword_ AND keyword_match_type = \'EXACT\' )',
	...
	CLOSE_VARIANTS : [
		...
		'    AND adgroup._LATEST_DATE = adgroup._DATA_DATE',
		'    AND keyword._LATEST_DATE = keyword._DATA_DATE',
		'  GROUP BY customer.ExternalCustomerId, AccountName, CampaignName,AdGroupId, CampaignId, AdGroupName, keyword.Criteria, Query',
		'  HAVING count > 1',
		')',
	...
	DUPLICATE_KEYWORD_IN_CAMPAIGN : [
		...
		' AND keyword._LATEST_DATE = keyword._DATA_DATE',
		' AND adgroup._LATEST_DATE = adgroup._DATA_DATE',
		'GROUP BY customer.ExternalCustomerId, AccountName, CampaignName, Criteria, KeywordMatchType, CampaignId',
		'HAVING count > 1',
		'ORDER BY count desc',
	].join( '\n' ),
	EXTENSION_NO_SITE_LINKS : [
		...
		'	)',
		'GROUP BY customer.ExternalCustomerId, campaign.CampaignName, CustomerDescriptiveName, AccountDescriptiveName, CampaignId',
		'HAVING TRUE',
	...
	INFORMATIONAL_SEARCH_QUERY : [
		...
		'GROUP BY customer.ExternalCustomerId, AccountName, CampaignName, AdGroupName, Criteria, Query, AdGroupId, CampaignId',
	].join( '\n' ),
	...
	KEYWORD_CAMPAIGN_MATCH_TYPE_MISMATCH : [
		...
		'	AND NOT IsNegative ',
		'	AND (',
		'		( KeywordMatchType != \'BROAD\' AND REGEXP_CONTAINS( CampaignName, \'BROAD\' ) )',
		'		OR',
		'		( KeywordMatchType != \'EXACT\' AND REGEXP_CONTAINS( CampaignName, \'EXACT\' ) )',
		'	)',
	].join( '\n' ),
	KEYWORD_ADGROUP_MATCH_TYPE_MISMATCH : [
		...
		'	AND NOT IsNegative ',
		'	AND (',
		'		( KeywordMatchType != \'BROAD\' AND REGEXP_CONTAINS( AdGroupName, \'BROAD\' ) )',
		'		OR',
		'		( KeywordMatchType != \'EXACT\' AND REGEXP_CONTAINS( AdGroupName, \'EXACT\' ) )',
		'	)',
	].join( '\n' ),
	...
	LOCATION_BID_MODIFIER_MISSING : [
		...
		'GROUP BY customer.ExternalCustomerId, AccountName, CampaignName, CampaignId',
		'HAVING TRUE',
	...
	MULTIPLE_DOMAINS_IN_ADGROUP : [
		...
		'  AND campaign._LATEST_DATE = campaign._DATA_DATE',
		'  AND customer._LATEST_DATE = customer._DATA_DATE',
		'group by customer.ExternalCustomerId, AccountName, CampaignName, AdGroupName, AdGroupId, CampaignId',
	...
	SEARCH_QUERIES_TRIGGERED_BY_MORE_THAN_ONE_KEYWORD : [
		...
		'      AND keyword._LATEST_DATE = keyword._DATA_DATE',
		'      AND sq._LATEST_DATE = sq._DATA_DATE',
		//'      AND DATE_DIFF( sq._LATEST_DATE, sq.Date, DAY ) <= 30', // not needed, because we take only 1 day now
		'    GROUP BY',
	...
	SHARED_NEGATIVE_KEYWORD_IN_AD : [
		...
		'	) as adgroup',
		'	ON ad.ExternalCustomerId = adgroup.ExternalCustomerId AND ad.CampaignId = adgroup.CampaignId AND ad.AdGroupId = adgroup.AdGroupId',
		'JOIN `' + BIGQUERY.PROJECT_ID + '.' + BIGQUERY.DATASET_ID + '.Customer_' + ACCOUNT_ID + '` as customer ON neg.ExternalCustomerId = customer.ExternalCustomerId',
		'    AND customer._LATEST_DATE = customer._DATA_DATE',
		'WHERE TRUE',
		'  AND (',
		'    FALSE',
...
function queryBigqueryAsync( query, projectId, datasetId, tableId, clusteringFields ){
	var job = {
		configuration: {
			query: {
				query : query,
...
function precomputeResults(){
	...
	views.forEach( function( viewName ){
		var schema = getTableSchema( VIEW_PREFIX + viewName );
		//schema : schema, // [ { name, type } ]
		var hasCampaignId = schema.map( _.property( 'name' ) ).indexOf( 'CampaignId' ) >= 0;
		var hasAdgroupId = schema.map( _.property( 'name' ) ).indexOf( 'AdGroupId' ) >= 0;
		
		var query = 'SELECT\n' +
			'\t\'' + viewName + '\' AS pitfall,\n' +
			'\tCAST( ExternalCustomerId AS STRING ) AS account_id,\n' +
			'\tCAST( ' + ( hasCampaignId ? 'CampaignId' : 'NULL' ) + ' AS STRING ) AS campaign_id,\n' +
			'\tCAST( ' + ( hasAdgroupId ? 'AdGroupId' : 'NULL' ) + ' AS STRING ) AS adgroup_id,\n' +
			'TO_JSON_STRING( STRUCT(';
		
		query += schema.map( _.property( 'name' ) )
			.filter( function( columnName ){
				return [ 'ExternalCustomerId', 'CampaignId', 'AdGroupId' ].indexOf( columnName ) == -1;
			})
			.join( ',\n' );
		...
		var job = queryBigqueryAsync( query,  BIGQUERY.PROJECT_ID, BIDDY_DATASET_ID, BIDDY_TABLE_NAME, clusteringFields );
...
function tableExists( tableId, retryCount ){
	projectId = BIGQUERY.PROJECT_ID;
	datasetId = BIGQUERY.DATASET_ID;
	...
	while( ! finished ){
		...
		try{
			...
			if( error + '' == 'Exception: Empty response' ){
				Logger.log( 'Got strange Bigquery-error: ' + error + '. pageToken is: ' + pageToken + '.' );
