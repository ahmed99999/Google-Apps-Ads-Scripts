...
var EXPECTED_DURATION_PER_CAMPAIGN_IN_SECONDS = 30;
...
function processAccount(){
	try{
		...
		// They may take to long and it is not necessary to wait for them.
		...
		*/
		...
		for( address in emailToAccountIdMap ){
			...
			if( newAdsForThisEmailAddress.length == 0 ){
				...
				var csv = formatCSV( newAdsForThisEmailAddress );
...
function formatCSV( matrix ){
	function replace( what, by ){ return function( item ){ return item.replace( what, by ) } };
	var separator = ',';
	var csv = 'sep=' + separator + '\n';
	csv += '"' + Object.keys( CONFIG.NEW_ADS_SCHEMA ).join( '"' + separator + '"' ) + '"\n';
	csv += matrix.map( function( row ){
		var res =
			'"' +
			row.map( replace( /"/g, '""' ) )
				.join( '"' +  separator + '"' )	+
