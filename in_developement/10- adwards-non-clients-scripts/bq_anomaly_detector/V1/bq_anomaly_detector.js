// --------- SETTINGS ----------------------
var SETTINGS_SHEET_URL = 'https://docs.google.com/spreadsheets/d/1WVAKcLaM5b6hnAq30netaTlQ59OjLelCWYcHb8DapWE/edit?usp=sharing';
...
var ANOMALY_VIEW_PREFIX = 'A_'; // must be distinct from SHARE_ANOMALY_VIEW_PREFIX
var SHARE_ANOMALY_VIEW_PREFIX = 'SA_'; // must be distinct from ANOMALY_VIEW_PREFIX

// Omit up to 2 digits before the decimal point for big values in emails.
var MIN_ROUND_TO = -2;

// Never display more than 2 digits after decimal point.
var MAX_ROUND_TO = 2;
...
var VIEWS = {
	...
	CONVERSION_LAG_VIEW : [
		...
		'  DayDiff,',
		'  ExternalCustomerId,',
		'  GREATEST( IFNULL( ROUND( ( Conversions - conv_the_day_before ) / NULLIF( Clicks, 0 ), 4 ), 0 ), 0 ) AS ExpectedAdditionalCR,',
		'  GREATEST( IFNULL( ROUND( ( ConversionValue - value_the_day_before ) / NULLIF( Clicks, 0 ), 4 ), 0 ), 0 ) AS ExpectedAdditionalValuePerClick',
		'FROM (',
		'  SELECT',
		'    ExternalCustomerId,',
		'    Conversions,',
		'    ConversionValue,',
		'    Clicks,',
		'    DATE_DIFF( RequestDate, Date, DAY ) AS DayDiff,',
		'    AVG( Conversions ) OVER ( PARTITION BY Date, AccountDescriptiveName, ExternalCustomerId ORDER BY RequestDate ROWS BETWEEN 1 PRECEDING AND 1 PRECEDING ) as conv_the_day_before,',
		'    AVG( ConversionValue ) OVER ( PARTITION BY Date, AccountDescriptiveName, ExternalCustomerId ORDER BY RequestDate ROWS BETWEEN 1 PRECEDING AND 1 PRECEDING ) as value_the_day_before,',
	...
	for( var accountName in alerts ){
		...
		accountAlerts.forEach( function( alert1 ){
			...
			html += '<tr class="regular">\
							...
							<td><span class="title">AVR</span>' + formatMetric( name, alert1.AvgHistoricValue, alert1.Currency )  + '</td>\
